...
class DataBindingIdlingResource : IdlingResource {
    ...
    private val idlingCallbacks
            = mutableListOf<IdlingResource.ResourceCallback>()

    // Give this a unique id to workaround an espresso bug where you
    // cannot register/unregister an idling resource w/ the same name.
    private val id = UUID.randomUUID().toString()

    // Holds whether isIdle is called and the result was false. We track
    // this to avoid calling onTransitionToIdle callbacks if Espresso
    // never thought we were idle in the first place.
